...
class Triangle {
    ...
    getPointFromTrilinearCoords(x: number, y: number, z: number): Point {
        ...
        return new Point(
            add1.x + add2.x + add3.x,
            add1.y + add2.y + add3.y,
        );
    }
    ...
    updatePoint(vertexidx: TriangleVertexIndex, newpos: Point): void {
        switch(vertexidx) {
            case 1: this.pt1 = newpos; break;
            case 2: this.pt2 = newpos; break;
            case 3: this.pt3 = newpos; break;
        }
    ...
    draw(context: CanvasRenderingContext2D, stroke: string = "black", fill: string = "white"): void {
        context.save();
...
class MorleyApp {
    ...
    mousePos(ev: MouseEvent | TouchEvent): Point {
        let mouseX = (ev as TouchEvent).changedTouches ?
            (ev as TouchEvent).changedTouches[0].pageX : (ev as MouseEvent).pageX;
        let mouseY = (ev as TouchEvent).changedTouches ?
            (ev as TouchEvent).changedTouches[0].pageY : (ev as MouseEvent).pageY;
