 ...
 */
function getAngle(vertex, pt1, pt2) {
...
class Triangle {
     ...
     */
    getPointFromTrilinearCoords(x, y, z) {
        let a = this.sideA;
        let b = this.sideB;
        let c = this.sideC;
    ...
    draw(context, stroke = "black", fill = "white") {
        context.save();
 ...
 */
function firstMorleyTriangle(tr) {
    // https://en.wikipedia.org/wiki/Morley%27s_trisector_theorem#Morley's_triangles
    return new Triangle(tr.getPointFromTrilinearCoords(1.0, 2.0 * Math.cos(tr.angleC / 3), 2.0 * Math.cos(tr.angleB / 3)), tr.getPointFromTrilinearCoords(2.0 * Math.cos(tr.angleC / 3), 1.0, 2.0 * Math.cos(tr.angleA / 3)), tr.getPointFromTrilinearCoords(2.0 * Math.cos(tr.angleB / 3), 2.0 * Math.cos(tr.angleA / 3), 1.0));
}
/**
...
class PointDragEvent {
    constructor(pointidx, mousestart, pointstart) {
...
class MorleyApp {
    constructor() {
        ...
        this.dragEventHandler = (ev) => {
            ...
            let mousepos = this.mousePos(ev);
            let relativepos = new Point(mousepos.x - this.canvas.offsetLeft, mousepos.y - this.canvas.offsetTop);
            this.triangle.updatePoint(de.pointidx, new Point(de.pointstart.x + (relativepos.x - de.mousestart.x), de.pointstart.y + (relativepos.y - de.mousestart.y)));
